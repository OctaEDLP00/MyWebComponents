export class BadgeGlassElement extends HTMLElement {
  size: Size = 'md'
  ...
  static define(tag = 'badge-glass', registry = customElements) {
    if (!registry.get(tag)) {
  ...
  render() {
    ...
    this.shadowRoot.setHTMLUnsafe(/* html */ `
			<style>${BadgeGlassElement.styles()}</style>
			<div class="badge glass ${this.size}">
				<!-- svg -->
				<slot>Glassmorphin</slot>
			</div>
		`)
  }
}
...
try {
  root.BadgeGlassElement = BadgeGlassElement.define()
} catch (e: unknown) {
  if (
