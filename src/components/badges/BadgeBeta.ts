export class BadgeBetaElement extends HTMLElement {
  size: Size = 'md'
  ...
  static define(tag = 'badge-beta', registry = customElements) {
    if (!registry.get(tag)) {
  ...
  render() {
    ...
    this.shadowRoot.setHTMLUnsafe(/* html */ `
			<style>${BadgeBetaElement.styles()}</style>
...
try {
  root.BadgeBetaElement = BadgeBetaElement.define()
} catch (e: unknown) {
  if (
