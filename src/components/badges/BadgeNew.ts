export class BadgeNewElement extends HTMLElement {
  size: Size = 'md'
  ...
  static define(tag = 'badge-new', registry = customElements) {
    if (!registry.get(tag)) {
  ...
  render() {
    ...
    this.shadowRoot.setHTMLUnsafe(/* html */ `
			<style>${BadgeNewElement.styles()}</style>
...
try {
  root.BadgeNewElement = BadgeNewElement.define()
} catch (e: unknown) {
  if (
