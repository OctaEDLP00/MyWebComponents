export class BadgeErrorElement extends HTMLElement {
  size: Size = 'md'
  ...
  static define(tag = 'badge-error', registry = customElements) {
    if (!registry.get(tag)) {
...
try {
  root.BadgeErrorElement = BadgeErrorElement.define()
} catch (e: unknown) {
  if (
